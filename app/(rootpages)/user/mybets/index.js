...
class MyBets extends Component{
    ...
    getallbets = (arg) => {
        ...
        const defrequest = fetch(
            "http://localhost:3000/getallbets",
            {
                method: 'POST',
    ...
    showDetails = (gameid, bettype) => {
        ...
        fetch(
            "http://localhost:3000/getspecificbetdetails",
            {
                method: 'POST',
    ...
    render(){
        return(
            <ScrollView ref={this.scrollViewRef} style={{backgroundColor:this.state.screenmode==='dark'?'#181818':'white', ...styles.containerView}} horizontal showsHorizontalScrollIndicator={false} decelerationRate={0} snapToInterval={Dimensions.get('window').width} snapToAlignment={"center"} scrollEnabled={true}>  
                                                ...
                                                <TouchableOpacity key={'open'+index} style={{display:this.state.loadingwarning===''?'flex':'none', flexDirection:'row', alignItems:'center', justifyContent:'center'}} onPress={()=>{game.status==='pending'?this.goToWR(game.gameid, game.bettype):this.goToDR(game.gameid, game.bettype)}}>
                                                    <View style={styles.betrow}>
                                                        ...
                                                        <View style={styles.bettext}>
                                                            <Text style={{fontFamily:'Chakra Petch Regular', fontSize:15, color:this.state.screenmode==='dark'?'#C8D1DB':'#646060'}}>{game.bettype==='h2h'?'Head 2 Head':'Admin'}</Text>
                ...
                <View style={{display:this.state.detailtype==='h2h', width:Dimensions.get('window').width, height:Dimensions.get('window').height*0.9, alignItems:'center'}}>
                    <View style={{display:!this.state.loading?'flex':'none', flexDirection:'row', alignItems:'center', justifyContent:'flex-start', marginTop:20, width:Dimensions.get('window').width}}>
                        ...
                        <Text style={{fontFamily:'Chakra Petch SemiBold', fontSize:24, marginLeft:108, color:this.state.screenmode==='dark'?'white':'black'}}>Game Details</Text>
                    </View>
                    ...
                    <View style={{width:Dimensions.get('window').width*0.85, marginTop:15}}>
                        <Text style={{fontFamily:'Chakra Petch SemiBold', fontSize:18, color:this.state.screenmode==='dark'?'white':'black', textAlign:'center'}}>{this.state.gamedetails.gametitle ? this.state.gamedetails.gametitle : ''}</Text>
                        <Text style={{fontFamily:'Chakra Petch Regular', fontSize:16, color:this.state.screenmode==='dark'?'white':'black', marginTop:10, textAlign:'center'}}>{this.state.gamedetails.gamedesc ? this.state.gamedetails.gamedesc : ''}</Text>
                    </View>

                    <Text style={{display:!this.state.reqloading && this.state.loadingwarning2===''?'flex':'none', fontFamily:'Chakra Petch Regular', fontSize:18, marginTop:20, color:this.state.screenmode==='dark'?'white':'black'}}>{this.state.gamedetails.creator?'This game was created by '+this.state.gamedetails.creator.username:''}</Text>
                ...
                <View style={{display:this.state.detailtype==='admin', width:Dimensions.get('window').width, height:Dimensions.get('window').height*0.9, alignItems:'center'}}>
                    ...
                    <View style={{display:!this.state.reqloading && this.state.loadingwarning2===''?'flex':'none', marginTop:9, width:Dimensions.get('window').width, alignItems:'center', height:150}}>
                        <View style={{position:'absolute', width:382, height:150, flexDirection:'column', alignItems:'center', justifyContent:'flex-start'}}>
                            ...
                            <View style={{marginTop:14, flexDirection:'row', alignItems:'center', justifyContent:'space-between', width:320, height:80}}>
                                <View style={{flexDirection:'column', alignItems:'center', justifyContent:'space-between', height:80}}>
                                    <Text style={{color:'white', fontFamily:'Chakra Petch SemiBold', fontSize:18}}>{this.state.gamedetails.bettype ? this.state.gamedetails.availablewagers[0].slice(0, -5) : ''}</Text>
                                    <Image source={require('./../../../../assets/player1.png')}/>
                                </View>
                                <View style={{flexDirection:'column', alignItems:'center', justifyContent:'space-between', height:80}}>
                                    <Text style={{color:'white', fontFamily:'Chakra Petch SemiBold', fontSize:18}}>{this.state.gamedetails.bettype ? this.state.gamedetails.availablewagers[1].slice(0, -5) : ''}</Text>
                                    <Image source={require('./../../../../assets/player2.png')}/>
                                </View>
                            </View>  
                        </View>
