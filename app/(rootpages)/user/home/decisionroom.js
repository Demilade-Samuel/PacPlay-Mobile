...
class DecisionRoom extends Component{
        ...
        if(gameid && gameid!=='undefined' && gameid!=='null'){
            ...
            if(data && data!==null){
                ...
                const defrequest = fetch(
                    "http://localhost:3000/drimpostercheck",
                    {
                        method: 'POST',
    ...
    cancelvote = () => {
        this.setState({reqloading:true, votewarning:''});
        fetch(
            "http://localhost:3000/cancelvote",
            {
                method: 'POST',
        ...
        return(
            <View style={{width:Dimensions.get('window').width, height:Dimensions.get('window').height, backgroundColor:this.state.screenmode==='dark'?'#181818':'white'}}>
                <View style={{display:!this.state.loading?'flex':'none', flexDirection:'row', alignItems:'center', justifyContent:'flex-start', marginTop:20}}>
                    <TouchableOpacity onPress={async ()=>{ this.state.socket.emit('leavedecisionroom', this.state.game.gameid); await AsyncStorage.removeItem('gameid');  navigation.navigate('/user/home');}}>
                    ...
                    <Text style={{fontFamily:'Chakra Petch SemiBold', fontSize:24, marginLeft:88, color:this.state.screenmode==='dark'?'white':'black'}}>Decision Room</Text>
                </View>
                
                <ScrollView ref={this.scrollViewRef} style={{ overflowX: 'none', maxHeight:Dimensions.get('window').height*0.9, backgroundColor:this.state.screenmode==='dark'?'#181818':'white'}} 
                    ...
                    <View style={{width:Dimensions.get('window').width, padding:24}}>
                        <View style={{flexDirection:'row', alignItems:'center', justifyContent:'space-between', width:Dimensions.get('window').width*0.9}}>
                            <Text style={{fontFamily:'Chakra Petch SemiBold', fontSize:22, color:this.state.screenmode==='dark'?'white':'black'}}>
                                My Vote
                            </Text>
                        ...
                        <Text style={{fontFamily:'Chakra Petch Regular', fontSize:18, color:this.state.screenmode==='dark'?'white':'black', marginTop:8}}>
                        ...
                        <View style={{width:Dimensions.get('window').width*0.9, flexDirection:'row', alignItems:'center', justifyContent:'space-around', marginTop:30}}>
                            <TouchableOpacity style={{borderRadius:8, backgroundColor:this.state.myvote==='I won'? this.state.screenmode==='dark'? '#1E9E40' : '#4285F4' : 'rgb(50,50,50)', flexDirection:'row', alignItems:'center', justifyContent:'center', width:160, height:180, flexDirection:'row', alignItems:'center', justifyContent:'center'}} onPress={()=>{this.setState({myvote:'I won'});}}>
                                <Text style={{color:'white', fontFamily:'Chakra Petch Regular', fontSize:18}}>I won</Text>

                            </TouchableOpacity>
                            
                            <TouchableOpacity style={{borderRadius:8, backgroundColor:this.state.myvote==='Opponent won'? this.state.screenmode==='dark'? '#1E9E40' : '#4285F4' : '#928E8E', width:160, height:180, flexDirection:'row', alignItems:'center', justifyContent:'center'}} onPress={()=>{this.setState({myvote:'Opponent won'});}}>
                        ...
                        <View style={{flexDirection:'row', alignItems:'center', justifyContent:'center', width:Dimensions.get('window').width*0.9, marginTop:20}}>
                            <TouchableOpacity style={{borderRadius:6, backgroundColor:this.state.myvote==='Draw'? this.state.screenmode==='dark'? '#1E9E40' : '#4285F4' : '#928E8E', height:56, width: Dimensions.get('window').width*0.5,  flexDirection:'row', alignItems:'center', justifyContent:'center'}} onPress={()=>{this.setState({myvote:'Draw'});}}>
                                <Text style={{color:'white', fontFamily:'Chakra Petch Regular', fontSize:16}}>End as a draw</Text>
                            </TouchableOpacity>
                        </View>

                        <Text style={{width:Dimensions.get('window').width*0.9, textAlign:'center', color:this.state.screenmode==='dark'?'white':'black', marginTop:15, fontFamily:'Chakra Petch Regular', fontSize:18}}>{this.state.game.votes ? this.state.game.votes.filter(x=>x!=='').length+' out of '+this.state.game.wagersidlist.length+' votes confirmed' : ''}</Text>
                    ...
                    <View style={{backgroundColor:this.state.screenmode==='dark'?'#181818':'white', width:Dimensions.get('window').width, height:Dimensions.get('window').height*0.9}}>
                        <View style={{width:Dimensions.get('window').width, padding:24, flexDirection:'row', alignItems:'center', justifyContent:'space-between'}}>
                            <Text style={{fontFamily:'Chakra Petch SemiBold', fontSize:18, color:this.state.screenmode==='dark'?'white':'black'}}>Game Details</Text>
                        ...
                        <View style={{paddingLeft:24, paddingRight:24, paddingTop:20, borderTopColor:'#928E8E', borderLeftColor:'rgba(0,0,0,0)', borderRightColor:'rgba(0,0,0,0)', borderBottomColor:'rgba(0,0,0,0)', borderWidth:1}}>
                            <Text style={{fontFamily:'Chakra Petch SemiBold', fontSize:18, color:this.state.screenmode==='dark'?'white':'black'}}>{this.state.game.gameid ? 'Game ID: '+this.state.game.gameid : 'Game ID:'}</Text>
                            <Text style={{fontFamily:'Chakra Petch SemiBold', fontSize:22, color:this.state.screenmode==='dark'?'white':'black', marginTop:25}}>Game Title</Text>
                            <Text style={{fontFamily:'Chakra Petch SemiBold', fontSize:20, color:this.state.screenmode==='dark'?'white':'black', marginTop:8}}>{this.state.game.gametitle ? this.state.game.gametitle : ''}</Text>
                            <Text style={{fontFamily:'Chakra Petch SemiBold', fontSize:22, color:this.state.screenmode==='dark'?'white':'black', marginTop:25}}>Game Description</Text>
                            <Text style={{fontFamily:'Chakra Petch Regular', fontSize:18, color:this.state.screenmode==='dark'?'white':'black', marginTop:8}}>{this.state.game.gamedesc ? this.state.game.gamedesc : ''}</Text>
                            <Text style={{fontFamily:'Chakra Petch SemiBold', fontSize:24, color:this.state.screenmode==='dark'?'white':'black', marginTop:25}}>{this.state.game.bettype==='h2h' ? 'Head 2 Head' : 'Admin'}</Text>
                            <Text style={{fontFamily:'Chakra Petch SemiBold', fontSize:26, color:this.state.screenmode==='dark'?'rgb(40, 120, 20)':'#124D07', marginTop:25}}>{'NGN '+this.state.game.stake}</Text>
                        </View>    
                    </View>
                    ...
                    <View style={{width:Dimensions.get('window').width, height:Dimensions.get('window').height, paddingTop:24, paddingBottom:24, paddingLeft:32, paddingRight:32, alignItems:'center'}}>
                        <Text style={{fontFamily:'Chakra Petch SemiBold', fontSize:20, color:this.state.screenmode==='dark'?'white':'black', textAlign:'center', marginTop:5}}>Final Agreement</Text>
                        <Text style={{display:this.state.winner==='draw'?'flex':'none', fontFamily:'Chakra Petch Regular', fontSize:18, color:this.state.screenmode==='dark'?'white':'black', textAlign:'center', marginTop:15}}>Do you agree to a draw?</Text>
                        <Text style={{display:this.state.winner!=='draw'?'flex':'none', fontFamily:'Chakra Petch Regular', fontSize:18, color:this.state.screenmode==='dark'?'white':'black', textAlign:'center', marginTop:15}}>{'Do you agree to '+(this.state.winner===''?'':this.state.winner)+' winning the bet?'}</Text>
                        ...
                        <Text style={{fontFamily:'Chakra Petch SemiBold', fontSize:16, color:this.state.screenmode==='dark'?'white':'black', marginTop:35, textAlign:'center'}}>{this.state.agreementwarning}</Text>
                ...
                <View style={{display:this.state.loading?'flex':'none', position:'absolute', top:50, left:0, height:Dimensions.get('window').height*0.9, width:Dimensions.get('window').width, backgroundColor:this.state.screenmode==='dark'?'#181818':'white', borderColor:'white', boderWidth:1, flexDirection:'column', alignItems:'center', justifyContent:'flex-start'}}>
