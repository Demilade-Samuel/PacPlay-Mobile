...
class DecisionRoom extends Component{
        ...
        if(gameid && gameid!=='undefined' && gameid!=='null'){
            ...
            if(data && data!==null){
                ...
                const defrequest = fetch(
                    "http://localhost:3000/drimpostercheck2",
                    {
                        method: 'POST',
    ...
    confirmdecision = () => {
        ...
        fetch(
            "http://localhost:3000/admindecision",
            {
                method: 'POST',
    ...
    cancelgame = () => {
        ...
        fetch(
            "http://localhost:3000/cancelgame2",
            {
                method: 'POST',
        ...
        return(
            <View style={{width:Dimensions.get('window').width, height:Dimensions.get('window').height, backgroundColor:this.state.screenmode==='dark'?'#181818':'white'}}>
                ...
                <ScrollView ref={this.scrollViewRef} style={{ overflowX: 'none', maxHeight:Dimensions.get('window').height*0.9, backgroundColor:this.state.screenmode==='dark'?'#181818':'white'}} 
                    ...
                    <View style={{display:this.state.game.creatorid? !this.state.loading && this.state.game.creatorid===this.state.userdata.userid ?'flex':'none':'none', backgroundColor:this.state.screenmode==='dark'?'#181818':'white', width:Dimensions.get('window').width, height:Dimensions.get('window').height*0.9, padding:24}}>
                        ...
                        <Text style={{fontFamily:'Chakra Petch Regular', fontSize:18, color:this.state.screenmode==='dark'?'white':'black', marginTop:15}}>
                        ...
                        <View style={{flexDirection:'column', alignItems:'center', justifyContent:'space-around', width:Dimensions.get('window').width-48}}>
                            <TouchableOpacity style={{ backgroundColor:'rgba(15,25,166,0.3)', borderRadius:6, padding:14, marginTop:15, width:320, alignItems:'center', borderWidth:1, borderColor:this.state.adnum===1?'yellow':'rgba(0,0,0,0)'}} onPress={()=>{ this.setState({adnum:1}); }}>
                                <Text style={{fontFamily:'Chakra Petch Regular', fontSize:18, color:this.state.screenmode==='dark'?'rgb(60,100,255)':'#0F19A6'}}>{this.state.game.availablewagers?this.state.game.availablewagers[0].slice(0, -4)+'won':''}</Text>
                            </TouchableOpacity>
                            <TouchableOpacity style={{backgroundColor:'rgba(244,25,25,0.3)', borderRadius:6, padding:14, marginTop:15, width:320, alignItems:'center', borderWidth:1, borderColor:this.state.adnum===2?'yellow':'rgba(0,0,0,0)'}} onPress={()=>{ this.setState({adnum:2}); }}>
                                <Text style={{fontFamily:'Chakra Petch Regular', fontSize:18, color:this.state.screenmode==='dark'?'rgb(255,80,80)':'#F41919'}}>{this.state.game.availablewagers?this.state.game.availablewagers[1].slice(0, -4)+'won':''}</Text>
                            </TouchableOpacity>
                    ...
                    <View style={{display:this.state.game.creatorid? !this.state.loading && this.state.game.creatorid!==this.state.userdata.userid ?'flex':'none':'none', backgroundColor:this.state.screenmode==='dark'?'#181818':'white', width:Dimensions.get('window').width, height:Dimensions.get('window').height*0.9, padding:24}}>
                            <View style={{flexDirection:'row', alignItems:'center', justifyContent:'space-between', width:Dimensions.get('window').width*0.9}}>
                                <Text style={{fontFamily:'Chakra Petch SemiBold', fontSize:17, color:this.state.screenmode==='dark'?'white':'black', width:250 }}>
                            ...
                            <Text style={{fontFamily:'Chakra Petch Regular', fontSize:18, color:this.state.screenmode==='dark'?'white':'black', textAlign:'center', width:Dimensions.get('window').width-48, marginTop:Dimensions.get('window').height*0.28}}>Awaiting the decision of the game admin. Please be patient...</Text>
                    </View>

                    <View style={{display:!this.state.loading?'flex':'none', backgroundColor:this.state.screenmode==='dark'?'#181818':'white', width:Dimensions.get('window').width, height:Dimensions.get('window').height*0.9}}>
                        ...
                        <View style={{paddingLeft:24, paddingRight:24, paddingTop:20, borderTopColor:'#928E8E', borderLeftColor:'rgba(0,0,0,0)', borderRightColor:'rgba(0,0,0,0)', borderBottomColor:'rgba(0,0,0,0)', borderWidth:1}}>
                            ...
                            <Text style={{fontFamily:'Chakra Petch SemiBold', fontSize:20, color:this.state.screenmode==='dark'?'white':'black', marginTop:20}}>Game Title</Text>
                            <Text style={{fontFamily:'Chakra Petch Regular', fontSize:18, color:this.state.screenmode==='dark'?'white':'black', marginTop:8}}>{this.state.game.gametitle ? this.state.game.gametitle : ''}</Text>
                            <Text style={{fontFamily:'Chakra Petch SemiBold', fontSize:20, color:this.state.screenmode==='dark'?'white':'black', marginTop:20}}>Game Description</Text>
                            <Text style={{fontFamily:'Chakra Petch Regular', fontSize:16, color:this.state.screenmode==='dark'?'white':'black', marginTop:8}}>{this.state.game.gamedesc ? this.state.game.gamedesc : ''}</Text>
                            <Text style={{fontFamily:'Chakra Petch SemiBold', fontSize:20, color:this.state.screenmode==='dark'?'white':'black', marginTop:20}}>{this.state.game.bettype==='h2h' ? 'Head 2 Head' : 'Admin'}</Text>
                            <Text style={{fontFamily:'Chakra Petch SemiBold', fontSize:22, color:this.state.screenmode==='dark'?'rgb(40, 120, 20)':'#124D07', marginTop:20}}>{'NGN '+this.state.game.stake}</Text>
                        </View> 

                        <Text style={{fontFamily:'Chakra Petch Regular', color:this.state.screenmode==='dark'?'white':'black', fontSize:18, paddingLeft:24, marginTop:10, marginBottom:15}}>
                        ...
                        <View style={{flexDirection:'row', alignItems:'flex-start', justifyContent:'space-around'}}>
                            <TouchableOpacity style={{ backgroundColor:'rgba(15,25,166,0.3)', borderRadius:6, padding:10, paddingLeft:15, paddingRight:15, paddingTop:8, paddingBottom:8, marginTop:10}}>
                                <Text style={{fontFamily:'Chakra Petch Regular', fontSize:16, color:this.state.screenmode==='dark'?'rgb(60,100,255)':'#0F19A6'}}>{this.state.game.availablewagers?this.state.game.availablewagers[0]:''}</Text>
                            </TouchableOpacity>
                            <TouchableOpacity style={{backgroundColor:'rgba(244,25,25,0.3)', borderRadius:6, padding:10, paddingLeft:15, paddingRight:15, paddingTop:8, paddingBottom:8,  marginTop:10}}>
                                <Text style={{fontFamily:'Chakra Petch Regular', fontSize:16, color:this.state.screenmode==='dark'?'rgb(255,80,80)':'#F41919'}}>{this.state.game.availablewagers?this.state.game.availablewagers[1]:''}</Text>
                            </TouchableOpacity>
