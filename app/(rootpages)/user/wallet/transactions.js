...
class Transactions extends Component{
    ...
    async componentDidMount(){
        ...
        this.setState({screenmode: screenmode, transactiontab: 'Deposits', userdata: userdata});
        this.getTransHistory('Deposit');
    }

    getTransHistory = (tab) => {
        this.setState({transactionerror:'', loading:true, transactiontab: tab})
        let url = tab==='Deposit' ? "http://localhost:3000/deposithistory" : "http://localhost:3000/withdrawalhistory";
        fetch(
            url,
            {
                method: 'POST',
    ...
    render(){
        return(
            <View style={{backgroundColor:this.state.screenmode==='dark'?'#181818':'white', width:Dimensions.get('window').width, height:Dimensions.get('window').height*0.94, padding:24,  borderWidth:1, borderColor:'black', alignItems:'center'}}>
                <View style={{display:'flex', flexDirection:'row', alignItems:'center', justifyContent:'flex-start', width:Dimensions.get('window').width*0.9}}>
                    ...
                    <Text style={{fontFamily:'Chakra Petch SemiBold', marginLeft:88, fontSize:20, width:Dimensions.get('window').width*0.9, color:this.state.screenmode==='dark'?'white':'black'}}>Transactions</Text>
                </View>
                ...
                <ScrollView style={{display:this.state.transactionerror==='' && !this.state.loading?'flex':'none', marginTop:10, width:Dimensions.get('window').width, paddingLeft:20, paddingRight:20, display:!this.state.loading?'flex':'none'}} contentContainerStyle={{alignItems:'center'}}>
                    {
                        this.state.transactions.length===0?
                            <Text style={{fontFamily:'Chakra Petch SemiBold', fontSize:18, marginTop:20, color:this.state.screenmode==='dark'?'white':'black'}}>{"You don't have any "+this.state.transactiontab.toLowerCase()+" transactions"}</Text>
